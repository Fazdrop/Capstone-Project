{{-- filepath: resources/views/staff/job_vacancy/edit.blade.php --}}
@extends('layouts.app')
...
@section('content')
    <div class="max-w-2xl mx-auto mt-10 bg-white rounded-xl shadow-lg border border-gray-200 p-8">
        ...
        <form action="{{ route('staff.job_vacancy.update', $job->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="division" class="w-full border rounded px-3 py-2"
                    value="{{ old('division', $job->division) }}">
                @error('division')
                    <div class="text-red-600 text-sm">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="description" class="w-full border rounded px-3 py-2" rows="4">{{ old('description', $job->description) }}</textarea>
                @error('description')
                    <div class="text-red-600 text-sm">{{ $message }}</div>
                @enderror
            </div>
            <div class="mb-4">
                ...
                <textarea name="requirements" class="w-full border rounded px-3 py-2" rows="3">{{ old('requirements', $job->requirements) }}</textarea>
                @error('requirements')
                    <div class="text-red-600 text-sm">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="date" name="deadline" class="w-full border rounded px-3 py-2"
                    value="{{ old('deadline', $job->deadline ? \Illuminate\Support\Carbon::parse($job->deadline)->format('Y-m-d') : '') }}">
                @error('deadline')
                    <div class="text-red-600 text-sm">{{ $message }}</div>
                @enderror
            </div>
            <div class="mb-6">
                ...
                <select name="is_active" class="w-full border rounded px-3 py-2">
                    ...
                    <option value="0" {{ old('is_active', $job->is_active) == '0' ? 'selected' : '' }}>Nonaktif
                    </option>
                </select>
                @error('is_active')
                    <div class="text-red-600 text-sm">{{ $message }}</div>
